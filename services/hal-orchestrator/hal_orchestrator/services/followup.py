...
def build_suggest_prompt(summary: str, when_local: str) -> str:
    return (
        ...
        "repeat, or 1-3 specific current options that fit this group (in NYC "
        "check nyc_events FIRST — real scraped events with links; otherwise "
        "use places or web_search so the suggestions are REAL — actual "
        "names, current, and include a link when you have one). Anchor to "
        "where these people actually live/meet (check the conversation — "
        "don't suggest a road-trip town they only visited once). Keep it "
        "light: one message, no pressure, no question barrage.\n\n"
