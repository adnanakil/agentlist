...
cache (hal_orchestrator.tools.weather).

The July 1–3 outage was the FORECAST layer (single Open-Meteo call, no ladder —
geocoding already had Nominatim → Open-Meteo). What's deterministic and breaks
silently if it regresses: the provider-ladder selection, the stale-cache TTL +
labeling, the met.no formatter (unit conversion, day bucketing), and that the
Open-Meteo happy-path format is byte-identical to the legacy output. Live HTTP
against the providers is not exercised here.
